...
// export async function run(
    ...
    const result = await sendToIndex('jetton/masters', {address: addressToString(jettonMinterAddress)}, provider);
    console.log("get result:", result)
    // write('Contract status: ' + result.status);
    console.log('Contract is mintable: ', result.jetton_masters[0].mintable);
    ...
    console.log("admin_address:", result.jetton_masters[0].admin_address)
    let parsedData = {
        supply: result.jetton_masters[0].total_supply,
        admin: Address.parseRaw(result.jetton_masters[0].admin_address),
        transfer_admin:Address.parseRaw(result.jetton_masters[0].admin_address)
    };
